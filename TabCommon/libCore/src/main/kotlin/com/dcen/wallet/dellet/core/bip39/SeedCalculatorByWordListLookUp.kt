...
class SeedCalculatorByWordListLookUp internal constructor(private val seedCalculator: SeedCalculator, wordList: WordList) {
    ...
    fun calculateSeed(mnemonic: Collection<CharSequence>, passphrase: String): ByteArray {
        ...
        val chars = mnemonic
                .map {
                    var wordChars: CharArray? = map[normalizer.normalize(it)]
                    if (wordChars == null) {
                        wordChars = normalizer.normalize(it).toCharArray()
